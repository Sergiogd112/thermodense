...
def create_figure_19(
    ...
    # F10.7 ranges for columns
    f107_ranges = [
        ("$\\bar{F}_{10.7}$ < 100", pl.col("f107") < 100),
        (
            "100 < $\\bar{F}_{10.7}$ < 160",
            (pl.col("f107") >= 100) & (pl.col("f107") < 160),
        ),
        ("$\\bar{F}_{10.7}$ > 160", pl.col("f107") >= 160),
    ]
    ...
    for row_idx, (df, mission_name) in tqdm(
        enumerate(zip(dfs, mission_names)), total=n_rows
    ):
        # Build column selection list
        cols_to_select = ["timestamp", "f107", msis_00_col, msis_20_col]
