/* Maintained local-first scientific figure-review workbench. */
import {
  COMMENT_TYPES,
  DECISIONS,
  VERDICTS,
...
function physicalPx(cm) {
  return physicalPixels(cm, effectivePhysicalScale());
}
...
async function loadData() {
  ...
  if (!res.ok) throw new Error(`figure set failed to load (${res.status})`);
  state.data = await res.json();
  for (const f of state.data.figures) {
    state.review.figures[f.id] = { decision: "unreviewed", comments: [] };
...
function persistDraft() {
  ...
  try {
    localStorage.setItem(
      draftStorageKey(state.data.figureSetVersion),
      JSON.stringify(buildManifest()),
    );
  } catch {
    // Review remains usable when local storage is unavailable.
  }
}

function restoreDraft() {
  try {
    const saved = localStorage.getItem(draftStorageKey(state.data.figureSetVersion));
    if (!saved) return;
    applyImportedManifest(JSON.parse(saved));
  } catch (error) {
    console.warn("Figure-review draft was not restored:", error);
  }
}
...
function applyImportedManifest(manifest, options = {}) {
  ...
  state.printWidths = imported.printWidths;
}
...
function setDecision(figId, decision) {
  ...
  r.decision = DECISIONS.includes(decision) ? decision : "unreviewed";
  persistDraft();
  render();
}
...
function header() {
  ...
  brand.innerHTML = "Figure review<span class='proto-tag'>LOCAL WORKBENCH</span>";
  bar.append(brand);
...
async function figureDetail(figId, opts = {}) {
  ...
  for (const p of f.panels) {
    ...
    const panelComments = state.review.figures[figId].comments.filter(
      (comment) => comment.target === `${figId}#${p.id}`,
    );
    ...
    btn.textContent = panelComments.length ? `annotate (${panelComments.length})` : "annotate";
    btn.onclick = () => {
    ...
    panelHeader.append(lbl, btn);
    pill.append(panelHeader);
    if (panelComments.length) pill.append(commentList(figId, p.id));
    panels.append(pill);
  }
  main.append(panels);
...
function boardColumn(col, figs) {
  ...
  section.ondrop = (e) => {
    ...
    if (id) {
      ...
      setDecision(id, col.toLowerCase());
    }
  };
  for (const f of figs) {
    const card = document.createElement("div");
    ...
    move.onchange = (e) => {
      e.stopPropagation();
      state.boardFocusColumn = DECISIONS.indexOf(move.value);
      setDecision(f.id, move.value);
    };
...
function renderB() {
  ...
  for (const f of state.data.figures) {
    ...
    bucket[d[0].toUpperCase() + d.slice(1)].push(f);
  }
...
function renderBModal() {
  ...
  modal.setAttribute("aria-label", state.compareSel.length === 2 ? "Compare figures" : "Figure detail");
  modal.tabIndex = -1;
  const close = document.createElement("button");
  close.className = "close";
  close.textContent = "✕";
  close.setAttribute("aria-label", "Close figure detail");
  close.onclick = () => {
    state.modalFigure = null;
    state.compareSel = [];
    render();
...
function buildManifest() {
  return buildReviewManifest(state.data, state.review, state.printWidths);
}
...
loadData().catch((error) => {
  const app = document.getElementById("app");
  app.textContent = `Figure set could not be loaded: ${error.message}`;
});
